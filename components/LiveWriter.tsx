...
const LiveWriter: React.FC<LiveWriterProps> = ({ book, progress }) => {
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-black overflow-hidden">
      ...
      {/* Left Panel: The Blueprint (Outline) - Always technical/dark */}
      <div className="w-full md:w-1/3 border-r border-ink-900 bg-paper-900 p-6 flex flex-col z-10 shadow-2xl">
        <div className="mb-6">
          <h2 className="text-gold-500 font-display text-xl mb-1">{book.title}</h2>
              ...
              className={`p-4 rounded border transition-all duration-500 ${
                chapter.isFinished 
                  ...
                  : idx === book.currentChapterIndex 
                    ? 'border-gold-500 bg-gold-500/10 scale-105 shadow-lg shadow-gold-900/20' 
                    : 'border-ink-800 bg-ink-900/50 text-ink-600'
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-mono uppercase">Chapter {chapter.number}</span>
                {chapter.isFinished ? <CheckCircle2 size={14} className="text-green-500"/> : 
                 idx === book.currentChapterIndex ? <Loader2 size={14} className="text-gold-500 animate-spin"/> : 
                 <Circle size={14} />}
              </div>
              <h3 className="font-serif font-bold text-sm mb-1">{chapter.title}</h3>
              <p className="text-xs line-clamp-2 opacity-70">{chapter.summary}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-ink-800">
           <div className="flex justify-between text-xs text-gold-500 mb-2 font-mono">
           ...
           <p className="text-center text-ink-600 text-[10px] mt-2 animate-pulse font-mono">
      ...
      <div 
        className={`flex-1 p-8 md:p-16 overflow-y-auto relative transition-colors duration-1000 ease-in-out ${theme.fonts.body}`}
        ref={scrollRef}
        style={{
        ...
        <div className="max-w-3xl mx-auto relative z-10 min-h-full">
          {book.status === 'planning' ? (
            <div className="flex flex-col items-center justify-center h-full space-y-6 opacity-70">
              <Loader2 className="w-16 h-16 animate-spin" style={{ color: theme.styles.accent }} />
              <p className={`text-2xl animate-pulse ${theme.fonts.display}`} style={{ color: theme.styles.accent }}>
                Consulting the Muses...
              </p>
            </div>
          ) : (
            <>
               <div className="text-center mb-12 border-b-2 pb-8" style={{ borderColor: theme.styles.borderColor }}>
                  <span className="text-xs font-sans font-bold tracking-[0.2em] uppercase mb-4 block opacity-60">Chapter {currentChapter?.number}</span>
                  <h1 className={`text-4xl md:text-5xl font-bold mb-4 ${theme.fonts.display}`} style={{ color: theme.styles.color }}>{currentChapter?.title}</h1>
               </div>

               <div className={`prose prose-lg max-w-none leading-relaxed text-lg ${theme.fonts.body}`} style={{ color: theme.styles.color }}>
               ...
               {currentChapter?.illustration && (
                 <div className="my-12 p-4 bg-white/5 shadow-xl rotate-1 transition-transform hover:rotate-0 duration-500">
                    <img 
                    ...
                    />
                    <p className="text-center text-xs font-sans mt-2 tracking-widest opacity-60">FIG {currentChapter.number}.1</p>
                 </div>
               )}
            </>
          )}
